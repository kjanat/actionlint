...
func (rule *RuleWorkflowCall) checkWorkflowCallCacheMode(pos *Pos, spec string, granted *CacheMode, m *ReusableWorkflowMetadata, checked map[workflowCacheModeVisit]bool) {
	...
	for _, id := range slices.Sorted(maps.Keys(m.JobCacheAccess)) {
		...
		if known && valid && want&^have != 0 {
			rule.Errorf(pos, "nested job %q of %q requests cache-mode %q but the calling job allows %q", id, spec, mode.Kind.String(), granted.Kind.String())
			continue
		}
		if access.Uses == "" {
			continue
		...
		rule.checkWorkflowCallCacheMode(pos, access.Uses, mode, next, checked)
	}
}
