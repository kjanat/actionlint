...
- Known snapshot versions also receive glob validation, whether supplied through a whole snapshot expression or a version expression. Diagnostics point to the source expression; unknown runtime versions remain deferred. Known empty image names and versions receive the same nonempty-value checks as literal fields.
...
- `cache-mode` uses that same one-pass string-literal folding. All four modes work at workflow and job level, including YAML aliases and reusable-workflow metadata; computed expressions and interpolation remain invalid.
...
- Known empty runner-label arrays are rejected in whole `runs-on` expressions, object `labels` fields, and standalone `labels` expressions. Group-only runner selection remains valid.

Runner-label conflict checks preserve correlations between matrix properties. They expand known original combinations, apply exclusions, and merge inclusions against the original axes. The runner's [matrix builder](https://github.com/actions/runner/blob/759385a3510197a58b5c08dc1f373b74b9f4643b/src/Sdk/WorkflowParser/Conversion/MatrixBuilder.cs#L553-L675) compares filter leaves with expression equality: strings compare without case, scalar types can be coerced, and nested mappings and sequences filter properties or indices. Matrix exclusion validation uses the same comparison. Duplicate matrix-value checks still distinguish strings from numbers. Expansion is bounded at 256 jobs; unknown or larger matrices retain label-name checks and conflicts against fixed labels without comparing unrelated alternatives.

Reusable-workflow metadata skips invalid, empty, nonscalar, and unsupported merge keys before recording duplicate root keys, matching the workflow parser's accepted-key order. File-first and AST-first cache tests verify identical cache ceilings and nested diagnostics while preserving the callee's own syntax errors.
...
The runner's [evaluated credentials schema](https://github.com/actions/runner/blob/759385a3510197a58b5c08dc1f373b74b9f4643b/src/Sdk/WorkflowParser/workflow-v1.0.json#L2686-L2702) permits omitted username/password fields and requires nonempty values for fields that are present. Its [credentials converter](https://github.com/actions/runner/blob/759385a3510197a58b5c08dc1f373b74b9f4643b/src/Sdk/WorkflowParser/Conversion/WorkflowTemplateConverter.cs#L1055-L1077) performs no completeness check. Evaluated empty and username-only credentials mappings remain accepted. Known plaintext passwords still receive the `credentials` diagnostic, including passwords inside whole container, services, or credentials expressions.

Actionlint requires an `image` property in evaluated container mappings; this is an explicit lint contract beyond the [runner schema](https://github.com/actions/runner/blob/759385a3510197a58b5c08dc1f373b74b9f4643b/src/Sdk/WorkflowParser/workflow-v1.0.json#L2601-L2624). An explicit empty job-container image remains accepted: the [converter returns no container](https://github.com/actions/runner/blob/759385a3510197a58b5c08dc1f373b74b9f4643b/src/Sdk/WorkflowParser/Conversion/WorkflowTemplateConverter.cs#L1203-L1218) for `{image: ""}`. Tests distinguish this disabling behavior from an omitted image property and retain the separate, stricter literal-form policy.
...
- Numeric bounds and finite timeout policies remain parser concerns. The existing positive-integer max-parallel lint policy also applies to known whole-strategy and per-field expression values; this is stricter than the runner's numeric conversion. Tests verify `0x1e`, `0o36`, `030`, and `3e1` all produce 30 for timeout-minutes. Nonfinite values are not asserted to be valid timeouts.
...
Known `fromJSON` and string-literal values are retained for constraints lost by type merging: every heterogeneous array element is checked, declared nonempty strings remain nonempty, and concurrency queue enum/conflict rules apply. Thus `ports: [80, {}]` cannot evade checks by merging its inferred element type to `any`. Unknown computed values continue to be deferred. Explicit empty job-container images retain the runner's disabling behavior; evaluated mappings still require the image property as described above.
