...
```sh
gh release download --repo kjanat/actionlint --pattern '*_linux_amd64.tar.gz' v1.12.0
tar xf actionlint_1.12.0_linux_amd64.tar.gz
./actionlint -version
```
...
```sh
gh attestation verify -R kjanat/actionlint actionlint_1.12.0_linux_amd64.tar.gz
```
...
example installs v1.12.0.

```sh
bash <(curl https://raw.githubusercontent.com/kjanat/actionlint/main/scripts/download-actionlint.bash) 1.12.0
```
