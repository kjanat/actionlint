...
Install whichever suits you, not several.
...
pushing, so they never need bumping by hand. `aur-prepare` fails loudly if a
`PKGBUILD` no longer contains a line it expects to rewrite, rather than pushing
one that still carries the previous release's version.
...
The rewrite logic has its own tests, run against these very `PKGBUILD` files so
