...
//
// This cannot go through GoReleaser's chocolateys pipe: that pipe declares
// choco as a required dependency and shells out to it, so it only runs on
// Windows, while the release itself runs on Linux. Adding a chocolateys block
// to .goreleaser.yaml would fail the release's preflight check. Instead this
// runs as a separate Windows job that reads the already-published release.
//
// The executable is embedded in the package rather than downloaded at install
// time. Chocolatey shims any .exe under tools/ automatically, so no install
// script is needed, and an embedded binary cannot break later if a release
// asset moves. Chocolatey's moderators require LICENSE.txt and
// VERIFICATION.txt alongside an embedded binary, so both are written too.
package main
...
func run() error {
	...
	// A prerelease is deliberately not pushed to the community feed: NuGet's
	// version grammar disagrees with semver on dotted prerelease identifiers,
	// and a release candidate has no business being the default install.
