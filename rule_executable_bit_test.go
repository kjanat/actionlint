...
func TestExecutableBitWorkflows(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"literal PATH chmod", "ubuntu-latest", "", "- run: chmod +x good.sh && ./bad.sh\n  env: {\"${{ 'PATH' }}\": tools}", ""},
		{"literal PATH direct", "ubuntu-latest", "", "- run: ./bad.sh\n  env: {\"${{ 'PATH' }}\": tools}", "bad.sh"},
		{"literal exported Bash function", "ubuntu-latest", "", "- run: chmod +x good.sh && ./bad.sh\n  env: {\"${{ 'BASH_FUNC_chmod%%' }}\": '() { return 0; }'}", ""},
