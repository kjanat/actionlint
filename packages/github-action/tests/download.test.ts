...
test('native download follows redirects through the runner proxy and honors NO_PROXY', { timeout: 5_000 }, async (t) => {
	...
	t.after(async () => {
		...
		await Promise.all([origin, proxy].map((server) => new Promise<void>((resolve, reject) => {
	...
	const proxied = await downloadVerified(download, 'http://proxy-only.invalid/redirect', join(directory, 'proxied'), digest);
